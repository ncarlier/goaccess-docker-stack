.SILENT :
.PHONY : mount update image rmi cleanup start stop wait rm shell logs
...
SHELL_FLAGS?=--rm -it --entrypoint="$(SHELL_BIN)" $(SHELL_CUSTOM_FLAGS)
# Default Docker run command for shell access
SHELL_CMD?=-c $(SHELL_BIN)
...
image: update
	...
	$(DOCKER) build --rm $(BUILD_CUSTOM_FLAGS) -t $(IMAGE) -f $(DOCKERFILE) .
	$(MAKE) cleanup
